...
I still need to confirm that none of the models with interaction effects are significant so I will compare those against this current model.
...
# Final reflections
I did find support for O'Connell et al.'s finding that adolescent female mating events had significantly higher mating scores than adult females. The methods seemed simple and straightforward when I initially read the paper. However, I did ran into a lot of unexpected problems throughout the replication assignment. I received the raw data from the first author because it was not originally made publically available. The raw file contained a lot of additional information that was not immediately relevant to my analyses. I cleaned up the data on my own, which may have contributed to the mismatch between my statistics and what was published. While working on the descriptive statistics, I realized that authors removed some lines of data, but I was unable to identify which ones because this was not described in the paper. This also potentially affected my GLMM results. My model summary values did not exactly match and I also found some models with interaction effects were significant, which O'Connell et al. did not find. I assume that the results from our analyses would be more aligned if a polished version of the data used was made available by the journal and authors. I also thought the paper could have included more detail about the GLMM, particularly in regards to which link function they used. This project highlighted the importance of being as specific as possible when describing methods in scientific publications. This contributes to its reproducibility and replicability. 
